...
대환대출 의사결정에 필요한 총비용을 정확하게 계산하고, 동시 요청과 재시도 상황에서도 금융상품 데이터가 일관되게 유지되도록 만든 Java와 Spring 기반 서비스입니다.
...
1. **계산 정확성:** 금융 계산에서 소수점 오차를 줄입니다.
2. **데이터 정합성:** 동시 갱신에서 앞선 변경이 사라지는 것을 막습니다.
3. **재시도 안전성:** 같은 요청이 다시 와도 중복 결과를 만들지 않습니다.
4. **성능과 정합성의 균형:** 조회 캐시를 적용하되 오래된 데이터가 제공될 가능성도 함께 다룹니다.
...
| 최소 화폐단위 `long` | 금액 연산 안정적 | 금리와 비율 계산이 복잡해짐 |
...
6. 인증과 인가, 관리자 권한 모델은 현재 범위에 포함하지 않았습니다.
...
금융 계산의 정확성을 먼저 확보하고, 동시 갱신과 재시도로 데이터가 잘못 저장되는 경로를 차단한 뒤 조회 성능을 개선했습니다.
